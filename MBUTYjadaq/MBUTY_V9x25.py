...
if plotMultiplicity is True:
   ...
   figmult, axmult = plt.subplots(num=401, nrows=2, ncols=len(digitID), sharey='row')
   axmult.shape    = (2,len(digitID))
...
#####################################      
    for ac in range(len(acqnum)):
        ...
        if plotMultiplicity is True:
           ...
           mywc = hh.hist1(multx,POPH[POPH[:,1]>=0,5],outBounds=True) # wires coinc
           
           my2Dwc = hh.hist2(multx,POPH[POPH[:,1]>=0,5],multx,POPH[POPH[:,1]>=0,6],outBounds=True) # wires coinc with strips 2D
...
#####################################    
    ...
    if plotMultiplicity is True:     
       ...
       multcumnorm   = multcum/np.sum(multcum[1:,:],axis=0)
       mult2Dcumnorm = mult2Dcum/np.sum(mult2Dcum)
       ...
       axmult[0][dd].bar(multx[:extentplot]- width,multcumnorm[:extentplot,0],width,label='w')
